...
export const find = async (req, res) => {
  try {
    let { page = 1, perPage = 10, column, sort, searchQuery } = req.query;
    ...
    const total = products.length > 0 ? await Product.countDocuments() : 0;
...
export const uploadDataToCSV = async (file, res) => {
  try {
    ...
    if (csvData.inValidData.length > 0) {
      ...
      return await Product.insertMany(csvData.data);
    }
